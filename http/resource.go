...
func writeFile(fs afero.Fs, dst string, in io.Reader, fileMode fs.FileMode, dirMode fs.FileMode) (os.FileInfo, error) {
	...
	file, err := fs.OpenFile(dst, os.O_RDWR|os.O_CREATE|os.O_TRUNC, fileMode)
	if err != nil {
		return nil, err
	}
	defer file.Close()
